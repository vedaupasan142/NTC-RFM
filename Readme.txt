Network Traffic Classification using Random Forest Model

Datasets:

1. Dataset 1 - from Open source - Dataset.csv
(https://www.kaggle.com/datasets/jsrojas/ip-network-traffic-flows-labeled-with-87-apps)

2. Dataset 2 - captured from Wireshark Locally - sample.csv

Code files - ipynb file format:

1. Code 1 - for Dataset 1 - Code1.ipynb
2. Code 2 - for Dataset 2 - Code2.ipynb
